...
def get_unique_glyphs_sorted() -> str:
    return "".join(sorted(set(char for string in UI_STRINGS.values() for char in string)))


def utf16_strlen(string: str) -> int:
...
def generate_c_header() -> str:
    ...
    return dedent(
        ...
        #ifndef SCRAN_UI_TEXT_H
        #define SCRAN_UI_TEXT_H
        ...
        struct scran_ui_strings {{
        {fields}
            char16_t unique_glyphs_sorted[{unique_glyphs_arr_len}];
        }};

        extern const struct scran_ui_strings g_ui_strings;
...
def generate_c_source(font_bytes: bytes, header_filename: str) -> str:
    ...
    array_contents = "\n".join(byte_rows)
    ui_string_initializers = "\n".join(
        "    .{} = {},".format(name, as_c_utf16_string_literal(ui_string))
        for name, ui_string in UI_STRINGS.items()
    )
    ...
    return dedent(
        ...
        const struct scran_ui_strings g_ui_strings = {{
        {ui_string_initializers}
            .unique_glyphs_sorted = {unique_glyphs_sorted},
        }};
        """
    ).format(
        header_filename=header_filename,
        array_contents=array_contents,
        ui_string_initializers=ui_string_initializers,
        unique_glyphs_sorted=as_c_utf16_string_literal(get_unique_glyphs_sorted()),
    )
